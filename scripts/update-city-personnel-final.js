...
// Şehir adlarını normalize et - Türkçe karakterleri koru
const normalizeCityName = (cityName) => {
  if (!cityName || cityName === 'null' || cityName === '#N/A') return null;
  const trimmed = cityName.trim();
  if (!trimmed) return null;
  // İlk harfi büyük, diğerlerini küçük yap
  // Türkçe karakterleri korumak için localeCompare kullan
...
const newCityPersonnel = Array.from(cityMap.values())
  ...
  .map((city, index) => {
    // Şehir adını normalize et (İlk harf büyük, diğerleri küçük)
    // Türkçe karakterleri koru
    let normalizedCity = city.city;
