...
const OrgCanvasInner = ({ onNodeClick, currentProjectId, currentProjectName, isPresentationMode = false }: OrgCanvasProps) => {
  ...
  // Önceki coordinator verilerini takip et (sonsuz döngüyü önlemek için)
  const prevCoordinatorDataRef = useRef<string>('')
  useEffect(() => {
    if (!viewPersonCard) {
      prevCoordinatorDataRef.current = ''
      return
    }
    
    const { type, coordinatorId, person } = viewPersonCard
    
    // Coordinator verilerini stringify et ve değişiklik kontrolü yap
    const coordinator = data.coordinators.find(c => c.id === coordinatorId)
    if (!coordinator) return
    
    const currentDataKey = type === 'coordinator' 
      ? JSON.stringify(coordinator.coordinator)
      : JSON.stringify(coordinator.deputies?.find(d => d.id === person.id || d.name === person.name))
    
    // Eğer veri değişmediyse güncelleme yapma (sonsuz döngüyü önle)
    if (currentDataKey === prevCoordinatorDataRef.current) return
    prevCoordinatorDataRef.current = currentDataKey
  ...
  }, [data.coordinators, viewPersonCard?.coordinatorId, viewPersonCard?.type, viewPersonCard?.person?.id])
      ...
      <div className="flex-1 h-full relative">
        ...
        <div className="absolute top-4 left-4 z-10 flex flex-col gap-2">
          <div className="flex items-center gap-2">
            ...
            {/* Personel butonu */}
            {/* TÜM PERSONEL BUTONU - ŞİMDİLİK KAPALI */}
            {/* <button
              onClick={() => {
            ...
            {/* <button
              ...
              title={turkeyMapOpen ? 'Haritayı Kapat' : 'Haritayı Aç'}
            >
              {turkeyMapOpen ? (
                <svg className="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                if (coordinator) {
                  updateCoordinator(personDetailModal.coordinatorId, {
                    coordinator: {
                      ...
                      name: updates.name !== undefined ? updates.name : coordinator.coordinator?.name || '',
                      title: updates.title !== undefined ? updates.title : coordinator.coordinator?.title || '',
                      university: updates.university !== undefined ? updates.university : coordinator.coordinator?.university || '',
                      department: updates.department !== undefined ? updates.department : coordinator.coordinator?.department || '',
                      hireDate: updates.hireDate !== undefined ? updates.hireDate : coordinator.coordinator?.hireDate || '',
                      seniority: updates.seniority !== undefined ? updates.seniority : coordinator.coordinator?.seniority || '',
                      jobDescription: updates.jobDescription !== undefined ? updates.jobDescription : coordinator.coordinator?.jobDescription || '',
                      jobDescriptionLink: updates.jobDescriptionLink !== undefined ? updates.jobDescriptionLink : coordinator.coordinator?.jobDescriptionLink || '',
                      email: updates.email !== undefined ? updates.email : coordinator.coordinator?.email || '',
                      phone: updates.phone !== undefined ? updates.phone : coordinator.coordinator?.phone || '',
                      notes: updates.notes !== undefined ? updates.notes : coordinator.coordinator?.notes || '',
                      cvFileName: updates.cvFileName !== undefined ? updates.cvFileName : coordinator.coordinator?.cvFileName || '',
                      cvData: updates.cvData !== undefined ? updates.cvData : coordinator.coordinator?.cvData || '',
                      photoData: updates.photoData !== undefined ? updates.photoData : coordinator.coordinator?.photoData || '',
                    }
                  })
                  showToast('Koordinatör bilgileri güncellendi', 'success')
                  // viewPersonCard useEffect tarafından otomatik güncellenecek
                }
                  ...
                  const updatedDeputies = coordinator.deputies?.map(d =>
                    d.id === personDetailModal.deputyId
                      ? {
                          ...
                          jobDescription: updates.jobDescription !== undefined ? updates.jobDescription : d.jobDescription || '',
